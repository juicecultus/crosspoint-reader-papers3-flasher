// manifest.js - reading the release manifest, and refusing a bad one.
//
...
//
// No DOM, no fetch, no browser API. Pure functions over text, so tests/run.js
// can drive every refusal from node.
...
export function parseManifest(text) {
	...
	try {
		...
		throw new ManifestError(`manifest: not valid JSON (${err.message})`);
	}
	...
	if (raw.product !== undefined) {
		must(raw.product === PRODUCT, `manifest: this is a ${raw.product} release, not an ${PRODUCT} one`);
	}

	must(typeof raw.version === 'string', 'manifest: version must be a string');
	must(VERSION_RE.test(raw.version), `manifest: version "${raw.version}" is not a version number`);
	...
	must(Object.keys(raw.artefacts).length > 0, 'manifest: the release lists no artefacts');
...
function readSize(value, where) {
	must(typeof value === 'number', `${where}: must be a number`);
	must(Number.isInteger(value), `${where}: must be a whole number of bytes`);
	must(value > 0, `${where}: must be greater than zero`);
	must(value <= 0xffffffff, `${where}: does not fit in the fastboot download command`);
	return value;
}

function readSha256(value, where) {
...
function readUrl(value, where) {
	must(typeof value === 'string' && value.length > 0, `${where}: must be a string`);
	if (value.startsWith('/')) {
		must(!value.startsWith('//'), `${where}: must not be protocol-relative`);
		return value;
	}
	must(value.startsWith('https://'), `${where}: must be https or an absolute path`);
	return value;
}
...
export function readArtefact(manifest, key) {
	...
	const artefact = {
		key,
		file: typeof a[`${key}_file`] === 'string' ? a[`${key}_file`] : null,
	...
	if (encoding !== undefined) {
		...
		artefact.encoding = encoding;
	}
	if (a[`${key}_plain_size`] !== undefined) {
		artefact.sizeUncompressed = readSize(a[`${key}_plain_size`], `artefacts.${key}_plain_size`);
	}
	if (a[`${key}_plain_sha256`] !== undefined) {
		artefact.sha256Uncompressed = readSha256(a[`${key}_plain_sha256`], `artefacts.${key}_plain_sha256`);
	}
	return artefact;
}
...
export function planArtefacts(profile, manifest, action) {
	...
	must(isPlainObject(manifest) && isPlainObject(manifest.artefacts),
		'manifest: artefacts must be an object');
	must(action === 'live' || action === 'install',
		`action must be live or install, got ${action}`);
...
export function checkDownloadSize(byteLength, maxDownloadSize) {
	must(typeof byteLength === 'number' && Number.isInteger(byteLength) && byteLength > 0,
		'download: byteLength must be a whole number greater than zero');
	must(typeof maxDownloadSize === 'string' && maxDownloadSize.length > 0,
		'download: the device did not report max-download-size');

	const limit = Number.parseInt(maxDownloadSize, maxDownloadSize.startsWith('0x') ? 16 : 10);
	must(Number.isInteger(limit) && limit > 0,
