// release.ts - fetching a release, and refusing bytes that are not the bytes
// the manifest names.
//
...
// so a page cannot fetch them cross-origin. This site answers /dl/:asset with
// an edge route that follows the redirect server side and streams the body back
// on the page's own origin, which makes the fetch same-origin and needs no CORS
// at all. src/app/dl/[asset]/route.ts is that route and carries the argument.

import type { DeviceProfile } from './profile.ts';

export interface FetchableArtefact {
  label: string;
  url: string;
  size: number;
  sha256: string;
...
export type FetchPhase = 'download' | 'decompress';
...
export async function fetchArtefact(
  ...
  if (!response.ok) {
    throw new ReleaseError(
      `${artefact.label}: the release server answered ${response.status}.`,
    );
  }
  if (!response.body) {
    throw new ReleaseError(
      `${artefact.label}: this browser gave no readable response body.`,
    );
  }
  ...
  if (onProgress) {
    onProgress({
      ...
      total: artefact.sizeUncompressed || 0,
    });
  }
  const stream = new Blob([downloaded as BlobPart])
    .stream()
    .pipeThrough(new DecompressionStream('gzip'));
  const plain = await readAll(
    stream,
    artefact.sizeUncompressed || 0,
    (done, total) => {
...
// is the shape the /dl route wants; an absolute https url is taken as it stands.
export function manifestUrl(profile: DeviceProfile): string {
...
export function resolveArtefactUrl(
