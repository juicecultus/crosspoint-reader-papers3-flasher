// fastboot.ts - the fastboot protocol over WebUSB, for this bootloader.
//
...
import type { DeviceDescription, DeviceProfile } from './profile.ts';
...
export type InfoHandler = (info: string) => void;
...
export class FastbootDevice {
  ...
  static filters(profile: DeviceProfile): USBDeviceFilter[] {
    return [
      { vendorId: profile.usb.vendorId, productId: profile.usb.productId },
    ];
  }
  ...
  async open(profile: DeviceProfile): Promise<void> {
    ...
    let found: { iface: USBInterface; alt: USBAlternateInterface } | null =
      null;
    ...
    if (this.endpointIn === null || this.endpointOut === null) {
      throw new FastbootError(
        'The fastboot interface has no bulk endpoint pair.',
      );
    }
  ...
  private async write(
    ...
    const result = await withTimeout(
      this.device.transferOut(this.endpointOut!, bytes as BufferSource),
      timeoutMs || DEFAULT_TIMEOUT_MS,
      'a write to the device',
    );
    if (result.status !== 'ok') {
      throw new FastbootError(
        `The device rejected a write (${result.status}).`,
      );
    }
    return result.bytesWritten;
  }

  private async readPacket(timeoutMs?: number): Promise<string> {
    const result = await withTimeout(
  ...
  async command(text: string, options?: CommandOptions): Promise<Reply> {
    const opts = options || {};
    const bytes = encoder.encode(text);
    if (bytes.length > MAX_COMMAND_BYTES) {
      throw new FastbootError(
        `Command longer than ${MAX_COMMAND_BYTES} bytes: ${text}`,
      );
    }
    await this.write(bytes, opts.timeoutMs);
    return this.readReply(opts.onInfo, opts.timeoutMs);
  }
  ...
  async tryGetvar(name: string): Promise<GetvarResult> {
    try {
      const reply = await this.command(`getvar:${name}`);
      if (reply.status === 'OKAY') {
  ...
  async download(
    ...
    if (reply.status !== 'DATA') {
      throw new FastbootError(
        `The device refused the download: ${reply.status}${reply.payload}`,
      );
    }
    ...
    while (sent < size) {
      ...
      // eslint-disable-next-line no-await-in-loop
      await this.write(bytes.subarray(sent, end), 60000);
      sent = end;
      if (onProgress) onProgress(sent, size);
    }

    const done = await this.readReply(null, 120000);
    if (done.status !== 'OKAY') {
      throw new FastbootError(
        `The device refused the transferred bytes: ${done.payload}`,
      );
    }
  }
  ...
  async flash(target: string, onInfo?: InfoHandler): Promise<void> {
    const reply = await this.command(`flash:${target}`, {
      onInfo,
      timeoutMs: 300000,
    });
    if (reply.status !== 'OKAY') {
      throw new FastbootError(
        `flash ${target} failed: ${reply.payload || 'refused'}`,
      );
    }
  }
  ...
  async boot(onInfo?: InfoHandler): Promise<void> {
    try {
      const reply = await this.command('boot', {
        onInfo,
...
export function browserSupport(
  ...
  if (!isSecureContext) {
    reasons.push(
      'This page is not on a secure connection, and WebUSB only works over https.',
    );
  }
  if (!usb) {
    if (/Firefox\//.test(ua)) {
      reasons.push(
        'Firefox does not implement WebUSB. Use Google Chrome or Microsoft Edge on a desktop computer.',
      );
    } else if (/Safari\//.test(ua) && !/Chrome\//.test(ua)) {
      reasons.push(
        'Safari does not implement WebUSB. Use Google Chrome or Microsoft Edge on a desktop computer.',
      );
    } else {
      reasons.push(
        'This browser does not have WebUSB. Use Google Chrome or Microsoft Edge on a desktop computer.',
      );
    }
  }
  if (usb && /Android/.test(ua)) {
    reasons.push(
      'Chrome on Android has WebUSB, but flashing wants a cable to a computer and a screen you can read while it runs. Use a desktop computer.',
    );
  }
  if (typeof DecompressionStream === 'undefined') {
    reasons.push(
      'This browser cannot decompress the system image (it has no DecompressionStream).',
    );
  }
  if (!(globalThis.crypto && globalThis.crypto.subtle)) {
    reasons.push(
      'This browser cannot check the download against its checksum (it has no Web Crypto).',
    );
  }
